...
def _rmse_objective(coeffs: np.ndarray, X_design: np.ndarray, y: np.ndarray) -> float:
    ...
    Returns:
        float: Root mean squared error [vehicles].
    """
...
def plot_convergence_curve(
    ...
    Args:
        convergence_history: RMSE recorded at each GD iteration [vehicles].
    ...
    assert len(convergence_history) > 0, (
        "[C3] convergence_history must not be empty."
    )
    ...
    ax.axhline(
        ...
        label=f"Final RMSE = {convergence_history[-1]:.4f} [vehicles]",
    )
...
def sensitivity_analysis(
    ...
    Satisfies the C3 framework requirement: "Include analysis of sensitivity —
    how does the solution change when a constraint is varied?"
    (Framework §5.2 C3: "Incluir análisis de sensibilidad")

    Args:
        model_coeffs:      Optimized coefficient vector from optimize_parameters().
        X:                 Feature array (n,) — normalised X used during optimization.
    ...
    # Postcondition: both lists must have the same length
...
def optimize_parameters(
    ...
    specified in the project pseudocode:
        ...
        2. Define parameter search space (GD from zeros; BFGS for comparison).
    ...
    Additionally runs sensitivity_analysis() automatically per framework §5.2 C3.

    Args:
        model_coeffs:            Coefficient vector from fit_regression() — used as
                                 reference; GD starts from zeros for a meaningful curve.
                                 Shape: (degree + 1,). Units: dimensionless.
        X:                       Feature array (n,) or (n, 1) — raw (un-normalised).
        ...
        learning_rate:           GD step size α. Range: (0, 1]. Default 1e-2.
        max_iterations:          Maximum GD iterations. Range: [1, +∞). Default 1000.
    ...
    Raises:
        AssertionError: If len(X) <= 10 (dataset too small) or degree not in {1,2,3}.
    ...
    # ── scipy BFGS — quasi-Newton comparison (framework §5.2 C3 required) ────
    ...
    if rmse_bfgs <= rmse_gd:
        ...
        print(f"[C3] ✓ BFGS selected              → RMSE = {rmse_bfgs:.4f} [vehicles]")
    else:
        optimal_params = theta_gd
        print(f"[C3] ✓ Gradient Descent selected  → RMSE = {rmse_gd:.4f} [vehicles]")
    ...
    # ── Step 4 (plot) — convergence curve saved automatically ─────────────────
    ...
    # ── Sensitivity analysis — required by framework §5.2 C3 ─────────────────
    sens_path = None
    if plot_save_path:
        sens_path = str(plot_save_path).replace(
            "convergence_curve", "sensitivity_analysis"
        )
...
if __name__ == "__main__":
    ...
    model_c2  = fit_regression(X_data, y_data, degree=DEGREE)
    coeffs_c2 = model_c2["coeffs"]
    print(f"[C2] Coefficients : {coeffs_c2}")
    print(f"[C2] R²           : {model_c2['r2']:.4f}")
    print(f"[C2] RMSE         : {model_c2['rmse']:.4f} [vehicles]")
